...
function entry(id: string, revision: string, load: ManifestEntry['load']): ManifestEntry {
...
test('one tool-call turn is fully recorded and projected into the next request', async () => {
  ...
  assert.deepEqual(adapter.requests[0]?.messages, [{ role: 'user', content: 'add 2 and 3' }])
  ...
  assert.deepEqual(
    adapter.requests[1]?.tools.map((tool) => tool.name),
    ['add'],
  )
  assert.deepEqual(
...
test('tool schemas are read live again before every model step', async () => {
  ...
  const selfRemoving = entry('remove-self', 'v1', () =>
    toolPlugin({
      name: 'remove_self',
...
test('tool replacement changes execution while preserving history and unrelated tools', async () => {
  ...
  const firstManifest = coreManifest(adapter, [entry('add', 'v1', () => addTool('add', 0)), echo])
  ...
  await host.reconcile(coreManifest(adapter, [entry('add', 'v2', () => addTool('add', 10)), echo]))
  await loop.run(session, 'second')

  const results = session.events.filter((event) => event.type === 'tool/result')
  assert.equal(
    results[0]?.type === 'tool/result' && results[0].ok ? results[0].output : undefined,
    2,
  )
  assert.equal(
    results[1]?.type === 'tool/result' && results[1].ok ? results[1].output : undefined,
    12,
  )
...
test('removing a tool withdraws its schema and logs missing and thrown results', async () => {
  ...
  const host = new DeclarativeHost()
  const removed = entry('removed', 'v1', () =>
    toolPlugin({
      name: 'removed',
  ...
  assert.deepEqual(
    registry.schemas().map((tool) => tool.name),
    ['boom'],
  )
  ...
  assert.deepEqual(
    results.map((event) => (event.type === 'tool/result' && !event.ok ? event.error : undefined)),
    ['tool "removed" is not registered', 'boom failed'],
  )
...
test('model replacement affects the next turn without replacing session history', async () => {
  const first = new ReplayModelAdapter('model-v1', [{ type: 'message', content: 'from v1' }])
  const second = new ReplayModelAdapter('model-v2', [{ type: 'message', content: 'from v2' }])
  const host = new DeclarativeHost()
...
test('failed model reload restores the previous adapter and keeps the loop runnable', async () => {
  ...
  const brokenManifest = stableManifest.map(
    (item): ManifestEntry =>
      item.id === 'model'
        ? entry('model', 'broken', () => ({
            ...modelPlugin(broken),
            setup() {
              throw new Error('broken model setup')
            },
          }))
        : item,
  )
  ...
  assert.equal(recoveredLast?.type === 'turn/end' ? recoveredLast.status : undefined, 'completed')
})
...
test('step limit records durable failure and releases the session run lock', async () => {
  ...
  await assert.rejects(loop.run(session, 'loop forever', { maxSteps: 2 }), StepLimitError)
  assert.deepEqual(
    session.events.slice(-2).map((event) => event.type),
    ['turn/error', 'turn/end'],
  )
  const failedLast = session.events.at(-1)
  assert.equal(failedLast?.type === 'turn/end' ? failedLast.status : undefined, 'failed')
