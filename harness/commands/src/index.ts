import { type CommandResult, snapshot } from '@deepseek-cordis/protocol'
...
function normalizeResult(value: unknown): CommandResult {
  ...
  if (
    ...
    const sourceSequence =
      'sourceSequence' in value && typeof value.sourceSequence === 'number'
...
export class InMemoryCommandRegistry implements CommandRegistry {
  ...
  async execute(
    ...
    const openTurns =
      session.events.filter((event) => event.type === 'turn/start').length -
    ...
    try {
      const commandNumber =
        session.events.filter((event) => event.type === 'command/run').length + 1
